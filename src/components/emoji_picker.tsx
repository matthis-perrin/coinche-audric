...
const emojiByCategory = (category: string): EmojiData[] => filteredEmojis.filter((e) => e.category === category);
const sortEmoji = (list: EmojiData[]): EmojiData[] => list.sort((a, b) => a.sort_order - b.sort_order);
...
export class EmojiSelector extends Component<EmojiSelectorProps, EmojiSelectorState> {
  ...
  //  RENDER METHODS
  //
  private readonly renderEmojiCell = ({item}: {item: {key: string; emoji: EmojiData}}): JSX.Element => (
    <EmojiCell
      key={item.key}
      emoji={item.emoji}
    ...
    return (
      <View style={styles.frame} {...other} onLayout={this.handleLayout}>
        <View style={styles.tabBar}>
          {showTabs && <TabBar activeCategory={category} onPress={this.handleTabSelect} theme={theme} width={width} />}
        </View>
        <View style={{flex: 1}}>
